...
public class OrderController {
    ...
    public ResponseEntity<OrderResponse> createOrder(@RequestBody OrderRequest orderRequest, HttpServletRequest request) {
        ...
        try {
            ...
            int amount = 0;
            for (Good good : orderRequest.getGoods()) {
            ...
            order = OrderRepository.createOrder(idempotencyKey);
            ...
            saga.addStep(new Saga.Step(
                    ...
                    () -> {
                        ...
                        try {
                            ...
                            MakePayment.makePayment(orderId, finalAmount);
                    ...
                    () -> {
                        ...
                        try {
                            ...
                            CancelPayment.cancelPayment(orderId);
            ...
            saga.addStep(new Saga.Step(
                    ...
                    () -> {
                        ...
                        ReserveGoods.reserveGoods(orderId, goodIds);
                        log.debug("inventory: end goods reservation.");
                    },
                    () -> {
                        log.debug("inventory: cancel goods reservation");
                        CancelGoodsReservation.cancelGoodsReservation(orderId);
                    }
            ));
            saga.addStep(new Saga.Step(
                    "reserve courier",
                    () -> {
                        ...
                        ReserveCourier.reserveCourier(orderId);
                        log.debug("shipment: end courier reservation.");
                    },
                    () -> {
                        log.debug("shipment: cancel courier reservation");
                        CancelCourierReservation.cancelCourierReservation(orderId);
                    }
            ));
